...
librarian::shelf(
  devtools, dplyr, DBI, DT, fs, glue, gt, here, htmltools, htmlwidgets, knitr, leaflet, leaflet.extras,
  offshorewindhabitat/offhabr,
  yogevherz/plotme, purrr, readr, sf, stringr, 
  tibble, tidyr, webshot,
  quiet = T)
...
d_spp |>
  select(-aphia_id) |> 
  count_to_treemap() |> 
  widget_webshot(species_all_treemap_png)
```

### Table

```{r}
#| label: fig-species_all_table
...
d_spp |>
  ...
  datatable(escape = F) |> 
  widget_webshot(species_all_table_png) # had to manually screenshot
```
...
```{r}
#| label: fig-species_extinction-risk-treemap
